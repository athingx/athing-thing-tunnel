...
import io.github.athingx.athing.aliyun.tunnel.core.protocol.*;
import io.github.athingx.athing.aliyun.tunnel.core.protocol.*;
import io.github.athingx.athing.aliyun.tunnel.core.util.GsonFactory;
...
public class TunnelProtocolCodec extends MessageToMessageCodec<WebSocketFrame, TunnelMessage> {
    ...
    @Override
    protected void encode(ChannelHandlerContext ctx, TunnelMessage tunnelMessage, List<Object> out) throws Exception {
        try {
            out.add(new BinaryWebSocketFrame(Unpooled.buffer().writeBytes(tunnelMessage.toBytes())));
        } catch (Throwable cause) {
            throw new TunnelProtocolCodecException("encode ra-protocol failure!", cause);
        }
    }
    ...
    protected void decode(ChannelHandlerContext ctx, WebSocketFrame frame, List<Object> out) throws Exception {
        try {
            ...
            throw new TunnelProtocolCodecException("decode ra-protocol failure!", cause);
        }
    ...
    private TunnelMessage.Body decodeBody(TunnelMessage.Header header, ByteBuf buf) throws TunnelProtocolCodecException {
        ...
        if (header.getLength() > buf.readableBytes()) {
            throw new TunnelProtocolCodecException(String.format("too large PAYLOAD-LENGTH: %s for BODY! message-id=%s",
                    header.getLength(),
                    header.getMessageId()
            ));
        }
        ...
        switch (header.getMessageType()) {
            ...
            default: {
                throw new TunnelProtocolCodecException(format("unsupported MESSAGE-TYPE: %s for BODY! message-id=%s",
